...
class gitModel extends model
{
     ...
     * @return int
     */
    public function saveCommits($repo, $branch, $logs, $version, $commentGroup, $accountPairs, $printLog)
    {
     ...
     * @return void
     */
    public function handleGitLabWebhook($data)
    {
        // Get repo by project ID
        $projectID = $data['project_id'];
        $repo = $this->dao->select('*')->from(TABLE_REPO)->where('serviceProject')->eq($projectID)->andWhere('SCM')->eq('Gitlab')->fetch();
        if(!$repo) return;
        ...
        if($data['object_kind'] == 'push')
        {
            ...
            foreach($commits as $commit)
            {
                ...
                // Save commit and link to objects
                $this->processWebhookCommit($repo, $branch, $log);
            }
        }
    }
     ...
     * @return void
     */
    public function handleGitHubWebhook($data)
    {
        // Get repo by project ID
        $repoID = $data['repository']['id'];
        $repo = $this->dao->select('*')->from(TABLE_REPO)->where('serviceProject')->eq($repoID)->andWhere('SCM')->eq('GitHub')->fetch();
        if(!$repo) return;
        
        // Handle push event
        if($data['event_type'] == 'push' || (isset($data['ref']) && strpos($data['ref'], 'refs/heads/') === 0))
        {
            $branch = str_replace('refs/heads/', '', $data['ref']);
            $commits = $data['commits'];
     ...
     * @return void
     */
    private function processWebhookCommit($repo, $branch, $log)
    {
        $this->loadModel('repo');
        
        // Check if commit already exists
        $existingCommit = $this->dao->select('id')->from(TABLE_REPOHISTORY)->where('repo')->eq($repo->id)->andWhere('revision')->eq($log->revision)->fetch('id');
        if($existingCommit) return;
        
        // Get latest version
        $lastCommit = $this->repo->getLatestCommit($repo->id);
        $version = isset($lastCommit->commit) ? (int)$lastCommit->commit + 1 : 1;
        
        // Save commit
        $version = $this->repo->saveOneCommit($repo->id, $log, $version, $branch);
        
        // Parse objects from commit message
        $objects = $this->repo->parseComment($log->msg);
        if($objects)
        {
            // Get account pairs
    ...
    /**
     * Get commits related to an object (story, bug, task).
     * 
     ...
     * @return array
     */
    public function getRelatedCommits($objectType, $objectID)
    {
        $this->loadModel('repo');
        return $this->repo->getCommitsByObject($objectType, $objectID);
    }
