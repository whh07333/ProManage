...
class actionModel extends model
{
    ...
    public function create($objectType, $objectID, $actionType, $comment = '', $extra = '', $actor = '', $autoDelete = true)
    {
        ...
        if($actionType == 'commented')
        {
            ...
            $this->parseMentions($actionID, $comment, $objectType, $objectID, $actor);
        }
        if($hasRecentTable)
        {
            $action->id = $actionID;
     ...
     * @return void
     */
    public function parseMentions($actionID, $comment, $objectType, $objectID, $actor)
    {
        ...
        if(!empty($matches[1]))
        {
            ...
            foreach($mentions as $account)
            {
                // Check if user exists
                $user = $this->dao->select('account')->from(TABLE_USER)->where('account')->eq($account)->fetch('account');
                if($user)
                {
                    // Save mention
                    $mention = new stdclass();
                    $mention->actionID   = $actionID;
                    $mention->account    = $account;
                    $mention->objectType = $objectType;
                    $mention->objectID   = $objectID;
                    $mention->createdBy  = $actor;
                    $mention->createdDate = helper::now();
                    $this->dao->insert(TABLE_ACTIONMENTION)->data($mention)->exec();
                    
                    // Send notification to mentioned user
                    $this->loadModel('message');
                    $object = $this->dao->select('*')->from($this->config->objectTables[$objectType])->where('id')->eq($objectID)->fetch();
                    $objectName = $object->name ? $object->name : $object->title;
                    $messageData = $actor . ' 在 ' . $this->lang->action->objectTypes[$objectType] . ' [' . $objectName . '] 中提及了你';
