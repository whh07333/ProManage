...
class messageModel extends model
{
     ...
     * Get messages.
     *
     * @param  string $status    all|wait|sended|read
     * @param  string $type      all|system|chat|mention|task
     * @param  string $orderBy
     * @access public
     * @return array
     */
    public function getMessages($status = 'all', $type = 'all', $orderBy = 'createdDate')
    {
        $now = helper::now();
        $query = $this->dao->select('t1.*')->from(TABLE_NOTIFY)->alias('t1')
        ...
        if(!empty($type) && $type != 'all') {
            $query->andWhere('t1.type')->eq($type);
        }
    ...
    /**
     * Get notification count by type and status.
     * 
     * @param  string $status
     * @param  string $type
     * @access public
     * @return int
     */
    public function getNotificationCount($status = 'wait', $type = 'all')
    {
        $now = helper::now();
        $query = $this->dao->select('COUNT(1) as count')->from(TABLE_NOTIFY)
    ...
    {
        return $this->dao->update(TABLE_NOTIFY)
            ->set('status')->eq('read')
     ...
     * @return bool
     */
    public function markAllAsRead($type = 'all')
    {
        $query = $this->dao->update(TABLE_NOTIFY)
            ->set('status')->eq('read')
            ->where('objectType')->eq('message')
            ->andWhere('toList')->eq(",{$this->app->user->account},")
    ...
    {
        $settings = $this->dao->select('value')->from(TABLE_USERSETTING)
            ->where('account')->eq($this->app->user->account)
            ->andWhere('module')->eq('message')
            ->andWhere('key')->eq('notificationSettings')
            ->fetch('value');
        
        return $settings ? json_decode($settings) : $this->getDefaultNotificationSettings();
    }
    ...
    {
        $exists = $this->dao->select('id')->from(TABLE_USERSETTING)
            ->where('account')->eq($this->app->user->account)
            ->andWhere('module')->eq('message')
            ->andWhere('key')->eq('notificationSettings')
            ->fetch('id');
        ...
        $data->module = 'message';
        $data->key = 'notificationSettings';
        $data->value = json_encode($settings);
        $data->type = 'json';
        $data->createdDate = helper::now();
        
        if($exists) {
            return $this->dao->update(TABLE_USERSETTING)->data($data)->where('id')->eq($exists)->exec();
        } else {
            return $this->dao->insert(TABLE_USERSETTING)->data($data)->exec();
        }
    }
