...
class todoTao extends todoModel
{
     ...
     * @return object
     */
    protected function fetch($todoID)
    {
     ...
     * @return array
     */
    protected function fetchRows($todoIdList)
    {
     ...
     * @return int
     */
    protected function getCountByAccount($account, $vision = 'rnd')
    {
     ...
     * @return array
     */
    protected function getProjectList($table, $idList)
    {
     ...
     * @return int
     */
    protected function insert($todo)
    {
     ...
     * @return bool
     */
    protected function updateRow($todoID, $todo)
    {
     ...
     * @return bool
     */
    protected function closeTodo($todoID)
    {
        $now = helper::now();
     ...
     * @return array
     */
    protected function getCycleList($todoList, $orderBy = 'date_asc')
    {
     ...
     * @return object
     */
    protected function buildCycleTodo($todo)
    {
     ...
     * @return false|string
     */
    protected function getCycleTodoDate($todo, $lastCycle, $today)
    {
        $date = '';
     ...
     * @return object
     */
    protected function getValidsOfBatchCreate($todos, $loop , $assignedTo)
    {
        $todo = new stdclass();
     ...
     * @return false|string
     */
    private function getCycleDailyTodoDate($todo, $lastCycle, $today)
    {
        $date = '';
     ...
     * @return bool
     */
    protected function updateDate($todoIdList, $date)
    {
     ...
     * @return int
     */
    protected function getTodoCountByAccount($account)
    {
        return $this->dao->select('COUNT(1) AS count')->from(TABLE_TODO)
            ->where('cycle')->eq('0')
            ->andWhere('deleted')->eq('0')
     ...
     * @return object
     */
    protected function setTodoNameByType($todo)
    {
     ...
     * @return array
     */
    protected function getListBy($type, $account, $status, $begin, $end, $limit, $orderBy, $pager = null)
    {
        return $this->dao->select('*')->from(TABLE_TODO)
            ->where('deleted')->eq('0')
            ->andWhere('vision')->eq($this->config->vision)
