...
<div class='page-actions text-center'>
  ...
  if($type == 'personal')
  {
      ...
      {
          extCommonModel::printLink('oa.leave', 'back', "id={$leave->id}", $lang->leave->back, "class='btn btn-primary loadInModal'");
      }
      $switchLabel = $leave->status == 'wait' ? $lang->leave->cancel : $lang->leave->commit;
      if(strpos(',wait,draft,', ",$leave->status,") !== false)
      {
          extCommonModel::printLink('oa.leave', 'switchstatus', "id={$leave->id}", $switchLabel, "class='btn btn-primary'");
      }
      if(strpos(',wait,draft,reject,', ",$leave->status,") !== false)
      {
          extCommonModel::printLink('oa.leave', 'edit',   "id={$leave->id}", $lang->edit,   "class='btn btn-primary loadInModal'");
          extCommonModel::printLink('oa.leave', 'delete', "id={$leave->id}", $lang->delete, "class='btn btn-primary deleteLeave'");
      }
  }
  else
  {
      if(strpos(',wait,doing,', ",$leave->status,") !== false)
      {
          ...
          extCommonModel::printLink('oa.leave', 'review', "id={$leave->id}&status=pass",   $lang->leave->statusList['pass'],   "class='btn btn-primary reviewPass'");
          extCommonModel::printLink('oa.leave', 'review', "id={$leave->id}&status=reject", $lang->leave->statusList['reject'], "class='btn btn-primary loadInModal'");
      }
      elseif($leave->status == 'back')
      {
          extCommonModel::printLink('oa.leave', 'review', "id={$leave->id}&status=pass&mode=back",   $lang->leave->statusList['pass'],   "class='btn btn-primary reviewPass'");
          extCommonModel::printLink('oa.leave', 'review', "id={$leave->id}&status=reject&mode=back", $lang->leave->statusList['reject'], "class='btn btn-primary loadInModal'");
      }
  }
