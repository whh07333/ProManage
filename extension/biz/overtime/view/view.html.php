...
<div class='page-actions text-center'>
  ...
  if($type == 'personal')
  {
      ...
      {
          extCommonModel::printLink('oa.overtime', 'switchstatus', "id=$overtime->id", $switchLabel, "class='switch-status btn btn-primary'");
      }
      if(strpos(',wait,draft,reject,', ",$overtime->status,") !== false)
      {
          extCommonModel::printLink('oa.overtime', 'edit',   "id=$overtime->id", $lang->edit,   "class='btn btn-primary loadInModal'");
          extCommonModel::printLink('oa.overtime', 'delete', "id=$overtime->id", $lang->delete, "class='btn btn-primary deleteOvertime'");
      }
  }
  elseif(strpos(',wait,doing,', ",$overtime->status,") !== false)
  {
      extCommonModel::printLink('oa.overtime', 'review', "id=$overtime->id&status=pass",   $lang->overtime->statusList['pass'],   "class='btn btn-primary reviewPass'");
      extCommonModel::printLink('oa.overtime', 'review', "id=$overtime->id&status=reject", $lang->overtime->statusList['reject'], "class='btn btn-primary loadInModal'");
  }
