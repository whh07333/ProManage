          ...
          <table align='center' border='0' cellpadding='0' cellspacing='0' width='600' style='border-collapse: collapse; background-color: #fff; border: 1px solid #cfcfcf; box-shadow: 0 0px 6px rgba(0, 0, 0, 0.1); margin-bottom: 20px; font-size:13px;'>
            ...
            <tr>
              <td style='padding: 10px; border: none;'>
                <?php if(!empty($this->lang->approval->mailContent[$method])):?>
                <?php if($method == 'revert' && $toNodeID == 'start'):?>
                <?php echo sprintf($this->lang->approval->mailContent[$method]['start'], $link, $title);?>
                <?php elseif($method == 'revert' && $isCreator):?>
                <?php $toNode = $this->dao->select('title')->from(TABLE_APPROVALNODE)->where('approval')->eq($approvalID)->andWhere('node')->eq($toNodeID)->fetch();?>
                <?php echo sprintf($this->lang->approval->mailContent[$method]['creator'], $link, $title, $toNode->title);?>
                <?php elseif($method == 'revert' && !$isCreator):?>
                <?php echo sprintf($this->lang->approval->mailContent['mail'], $link, $title);?>
                <?php else:?>
                <?php echo sprintf($this->lang->approval->mailContent[$method], $link, $title);?>
                <?php endif;?>
                <?php elseif($approval->result):?>
