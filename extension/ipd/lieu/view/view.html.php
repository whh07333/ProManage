...
<div class='page-actions text-center'>
  ...
  if($type == 'personal')
  {
      ...
      {
          extCommonModel::printLink('oa.lieu', 'switchstatus', "id={$lieu->id}", $switchLabel, "class='btn btn-primary'");
      }
      if(strpos(',wait,draft,reject,', ",$lieu->status,") !== false)
      {
          extCommonModel::printLink('oa.lieu', 'edit',   "id={$lieu->id}", $lang->edit,   "class='btn btn-primary loadInModal'");
          extCommonModel::printLink('oa.lieu', 'delete', "id={$lieu->id}", $lang->delete, "class='btn btn-primary deleteLieu'");
      }
  }
  elseif(strpos(',wait,doing,', ",$lieu->status,") !== false)
  {
      extCommonModel::printLink('oa.lieu', 'review', "id={$lieu->id}&status=pass",   $lang->lieu->statusList['pass'],   "class='btn btn-primary reviewPass'");
      extCommonModel::printLink('oa.lieu', 'review', "id={$lieu->id}&status=reject", $lang->lieu->statusList['reject'], "class='btn btn-primary loadInModal'");
  }
