...
class imChat extends model
{
    ...
    public function getPublicList($userID)
    {
        ...
        $chats = $this->dao->select('*')->from(TABLE_IM_CHAT)
            ...
            ->andWhere("(dismissDate is null or dismissDate = null)")
            ->andWhere("(mergedDate is null or mergedDate = null)")
            ->andWhere("(archiveDate is null or archiveDate = null)")
    ...
    public function getGidListByUserID($userID = 0, $includeMerged = false)
    {
        ...
        $gidList = $this->dao->select('t1.gid')->from(TABLE_IM_CHAT)->alias('t1')
            ...
            ->andWhere("(t2.quit is null or t2.quit = null)", $includeMerged)
            ->beginIF($includeMerged)->orWhere("(t1.mergedDate is not null and t1.mergedDate != null)")->markRight()->fi()
            ->fetchPairs('gid');
    ...
    public function getListByUserID($userID = 0)
    {
        ...
        $chats = $this->dao->select('chat.*, cu.star, cu.hide, cu.mute, cu.freeze, cu.category, cu.lastReadMessage, cu.lastReadMessageIndex')
            ...
            ->andWhere("(cu.quit is null or cu.quit = null)")
            ->andWhere("(chat.dismissDate is null or chat.dismissDate = null)", true)
...
return in_array($account, $adminArray);
    ...
    public function getUserPairs($gid = '')
    {
        $userIdList = $this->dao->select('user')->from(TABLE_IM_CHATUSER)
            ->where("(quit is null or quit = null)")
...
foreach(array('dismissDate', 'mergedDate', 'archiveDate') as $nullable) if(isset($chat->$nullable) && empty($chat->$nullable)) $chat->$nullable = null;
    ...
    public function join($gid = '', $userID = 0)
    {
        ...
        if($data)
        {
            ...
            if($data->quit == null || empty($data->quit)) return $userID;
    ...
    public function format($chats, $getLastMessage = false)
    {
        ...
        foreach($chats as $chat)
        {
            ...
            $chat->createdDate     = empty($chat->createdDate) || $chat->createdDate == null ? 0 : strtotime($chat->createdDate);
            $chat->editedDate      = empty($chat->editedDate) || $chat->editedDate == null ? 0 : strtotime($chat->editedDate);
            $chat->lastActiveTime  = empty($chat->lastActiveTime) || $chat->lastActiveTime == null ? 0 : strtotime($chat->lastActiveTime);
            $chat->dismissDate     = empty($chat->dismissDate) || $chat->dismissDate == null ? 0 : strtotime($chat->dismissDate);
            $chat->mergedDate      = empty($chat->mergedDate) || $chat->mergedDate == null ? 0 : strtotime($chat->mergedDate);
            $chat->archiveDate     = empty($chat->archiveDate) || $chat->archiveDate == null ? 0 : strtotime($chat->archiveDate);
    ...
    public function getMembers($chat, $trueOnSystemChat = false)
    {
        ...
        else
        {
            $memberList = $this->dao->select('user')->from(TABLE_IM_CHATUSER)->alias('tcu')
                ...
                ->where("(tcu.quit is null or tcu.quit = null)")
    ...
    public function getMemberDetails($cgid, $pager = null, $orderBy = '', $memberIDs = array())
    {
        ...
        $memberList = array_map(function($member)
        {
            ...
            $member->join     = empty($member->join) || $member->join == null ? 0 : strtotime($member->join);
            $member->lastSeen = empty($member->lastSeen) || $member->lastSeen == null ? 0 : strtotime($member->lastSeen);
            $member->lastPost = empty($member->lastPost) || $member->lastPost == null ? 0 : strtotime($member->lastPost);
            return $member;
        }, $memberList);
...
$accountAdmin->admin = in_array($accountAdmin->account, $sysAdminArray) ? 'super' : '';
        ...
        if($chat->mergedDate != null && !empty($chat->mergedDate)) return false;
    ...
    public function getNextOwnerCandidate($chat, $userID, $asAccount = true)
    {
        if(!empty($chat->admins))
        {
            $seniormostAdmin = $this->dao->select($asAccount ? 'account' : 'user')->from(TABLE_IM_CHATUSER)->alias('tcu')
                ...
                ->andWhere("(tcu.quit is null or tcu.quit = null)")
    ...
    public function pruneExpired()
    {
        ...
        $expiredGroups = $this->dao->select('gid')
            ->from(TABLE_IM_CHAT)
            ->where('type')->eq('group')
            ->andWhere("(dismissDate is not null and dismissDate != null)")
    ...
    public function search($searchField, $pager, $orderBy)
    {
        ...
        $chatMemberCounts = $this->dao->select('tc.gid, COUNT(*) AS memberCount')
            ...
            ->andWhere("(tc.dismissDate is null or tc.dismissDate = null)")
            ->andWhere("(tc.mergedDate is null or tc.mergedDate = null)")
            ->andWhere("(tcu.quit is null or tcu.quit = null)")
