...
class imUser extends model
{
    ...
    public function identifyWithToken($account, $token, $device = '')
    {
        ...
        $userTokens = $this->dao->select('t1.*, t2.device, t2.token, t2.validUntil')->from(TABLE_USER)->alias('t1')
                ...
                ->orWhere("(t2.validUntil is null or t2.validUntil = null)")->markRight(1)
                ->fetchAll();
        ...
        foreach($userTokens as $userToken)
        {
            ...
            {
                if(!empty($userToken->locked) && $userToken->locked != null)
                {
...
$user->admin = in_array($user->account, $adminArray) ? 'super' : '';
    ...
    {
        /* 安全修复：验证 deviceID 格式，防止 SQL 注入 */
        /* deviceID 应该是 16 位的 MD5 码，只允许 a-f、A-F、0-9 */
        if(!empty($deviceID) && !preg_match('/^[a-fA-F0-9]{16}$/', $deviceID))
        {
            dao::$errors[] = 'Invalid deviceID format';
            return false;
        }
