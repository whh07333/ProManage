...
$lang->admin->setModuleNotice = '请先在"功能开关"中打开"%s"功能,才能进行配置。';
...
if(helper::hasFeature('auditplan'))      $lang->admin->menuList->feature['tabMenu']['project']['nc']            = array('link' => "质量保证|custom|set|module=nc&fieldList=typeList", 'exclude' => 'custom');
...
{
    $lang->admin->menuList->projectflow['childMenu']['deliverable'] = array('link' => '交付物类型|deliverable|browse|id=%s', 'subModule' => ',tree,');
    $lang->admin->menuList->projectflow['menuOrder']['20'] = 'deliverable';
}

if(helper::hasFeature('process')) $lang->admin->menuList->projectflow['childMenu']['report'] = array('link' => '流程概览|workflowgroup|report|id=%s');
if(helper::hasFeature('auditplan') || helper::hasFeature('deliverable') || helper::hasFeature('cm') || helper::hasFeature('change')) $lang->admin->menuList->projectflow['childMenu']['setmodule'] = array('link' => '功能开关|workflowgroup|setmodule|id=%s');
if(helper::hasFeature('process'))     $lang->admin->menuList->projectflow['childMenu']['process'] = array('link' => '过程定义|process|browse|id=%s', 'subModule' => 'process,activity');
if(helper::hasFeature('auditplan'))   $lang->admin->menuList->projectflow['childMenu']['auditcl'] = array('link' => '质量保证|auditcl|browse|id=%s', 'subModule' => 'auditcl');
if(helper::hasFeature('deliverable')) $lang->admin->menuList->projectflow['childMenu']['review']  = array('link' => '评审流程|review|admin|id=%s', 'subModule' => 'review,reviewcl');
...
{
    $lang->admin->menuList->projectflow['childMenu']['process']['subMenu']['process']  = array('link' => '过程列表|process|browse|id=%s', 'subModule' => 'process');
    $lang->admin->menuList->projectflow['childMenu']['process']['subMenu']['activity'] = array('link' => '活动列表|activity|browse|id=%s', 'subModule' => 'activity');
}

if(helper::hasFeature('deliverable'))
{
    $lang->admin->menuList->projectflow['childMenu']['review']['subMenu']['review']   = array('link' => '评审流程|review|admin|id=%s');
    $lang->admin->menuList->projectflow['childMenu']['review']['subMenu']['cllist']   = array('link' => '检查清单|reviewcl|browse|id=%s', 'subModule' => 'reviewcl', 'exclude' => 'reviewcl-setcategory');
    $lang->admin->menuList->projectflow['childMenu']['review']['subMenu']['category'] = array('link' => '检查单分类|reviewcl|setcategory|id=%s', 'subModule' => 'custom');
}
