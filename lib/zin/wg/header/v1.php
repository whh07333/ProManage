...
class header extends wg
{
    ...
    public static function chatBar()
    {
        ...
        $rooms = $app->dbh->query("SELECT DISTINCT r.id FROM " . TABLE_CHATROOM . " r LEFT JOIN " . TABLE_CHATROOMMEMBER . " m ON r.id=m.roomID WHERE m.account = '$account' OR r.createdBy = '$account'")->fetchAll();

        foreach($rooms as $room)
        {
            $member = $app->dbh->query("SELECT lastRead FROM " . TABLE_CHATROOMMEMBER . " WHERE roomID = " . (int)$room->id . " AND account = '$account'")->fetch();
            $lastReadID = $member && $member->lastRead ? (int)$member->lastRead : 0;

            $unread = $app->dbh->query("SELECT COUNT(*) as count FROM " . TABLE_CHATMESSAGE . " WHERE roomID = " . (int)$room->id . " AND id > " . $lastReadID)->fetch();
            if($unread) $unreadCount += (int)$unread->count;
        }

        if($unreadCount > 99) $unreadCount = '99+';

        $chatURL = helper::createLink('chat', 'index', '', '', true);
        ...
        (
            ...
            set::size('sm'),
            set::icon('chat'),
            $unreadCount ? label(setClass('danger label-dot absolute'), set::style(array('top' => '-3px', 'right' => '-3px', 'min-width' => '18px', 'height' => '18px', 'border-radius' => '9px')), $unreadCount) : null,
            set('data-toggle', 'modal'),
            set('data-type', 'ajax'),
            set('data-url', $chatURL),
            set('data-size', 'lg'),
            set('data-title', '职聊')
        );
    }
