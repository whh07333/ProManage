...
class Word2007 extends AbstractWriter implements WriterInterface
{
     ...
     */
    private function addHeaderFooterMedia($zip, $docPart)
    {
     ...
     * @param int &$rId
     */
    private function addHeaderFooterContent(&$section, $zip, $elmType, &$rId)
    {
     ...
     */
    private function addNotes($zip, &$rId, $noteType = 'footnote')
    {
        $phpWord = $this->getPhpWord();
     ...
     * @param int &$rId
     */
    private function addComments($zip, &$rId)
    {
        $phpWord = $this->getPhpWord();
     ...
     * @param int &$rId
     */
    private function addChart($zip, &$rId)
    {
        $phpWord = $this->getPhpWord();
